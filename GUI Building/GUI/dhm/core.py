...
class HoloGram:
    ...
    def set_background_img(self, read_path_back: str = '', bgd_file_name: str = 'ref'):
        if os.path.isfile(read_path_back + bgd_file_name + '.tiff'):
            self.BACKGROUND = np.array(plt.imread(read_path_back + bgd_file_name+ '.tiff')[:, :], dtype=float)
    ...
    def filter_background_process(self):
        if not self.ROI_enable:
            self.Order_Filter = dhm.interactive.filter_fixed_point(hologram_raw=self.HOLOGRAM,
                                                               quadrant=self.filter_quadrant_main,
                                                               filter_rate=self.filter_rate_main,
                                                               filter_type=self.filter_type_main)

            background_filtered = dhm.utils.fourier_process(self.BACKGROUND, self.Order_Filter)
    ...
    def hologram_process(self):
        ...
        hologram = dhm.utils.fourier_process(self.HOLOGRAM, self.Order_Filter) * self.Processed_Background
