...
class HoloGram:
    ...
    Fourier_Filter = np.ndarray(shape=(3000, 4000), dtype=np.uint16)
    ...
    def set_img_show(self, img_to_show):
        img_show = self.HEIGHT_MAP
        if img_to_show == "Height map":
            img_show = self.HEIGHT_MAP
        elif img_to_show == "FFT Filter":
            img_show = np.log(1 + np.abs(self.Fourier_Filter))
        elif img_to_show == "Phase map":
            img_show = self.PHASE_MAP
        elif img_to_show == "Wrapped Phase":
            img_show = self.WRAPPED_PHASE
        elif img_to_show == "Hologram":
            img_show = self.HOLOGRAM
        elif img_to_show == "Reference":
            img_show = self.BACKGROUND
        return img_show
    ...
    def filter_background_process(self):
        if not self.ROI_enable:
            self.Fourier_Filter = self.filter_fixed_point(hologram_raw=self.HOLOGRAM,
                                                          quadrant=self.filter_quadrant_main,
                                                          filter_rate=self.filter_rate_main,
                                                          filter_type=self.filter_type_main)
            background_filtered = self.fourier_process(self.BACKGROUND, self.Fourier_Filter)
