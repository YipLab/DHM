...
class HoloGram:
	...
	def holo_roi_show(self):
		print(f"Currently viewing ROI: (left, right, top, bottom) = ({self.left}, {self.right}, {self.top}, {self.bot})",
	...
	def process_roi(self, filter_quadrant: int = 3, num_data: int = 1, k_factor=1.5, vertical_step: int = 2):
		...
		circle_filter = dhm.interactive.filter_fixed_point(self.HOLOGRAM[0][self.left: self.right, self.top: self.bot],filter_quadrant)
		...
		for data in range(num_data):
			...
			phase_reconstructed, diffraction_distance, reconstructed, intensity_final = dhm.utils.reconstruction_angular(self, holo_apd, vertical_step)
